classdef hh_ms_test < matlab.unittest.TestCase
...
methods 
   function [hhS, countryS, priceS, paramS, cS] = modelSetup(testCase, setNo)
      % setNo = testCase.setNo;
      cS = const_ms(setNo);
...
methods (Test)
   %%  OJT: x and nh
   function xhTest(testCase, setNo)

      [hhS, countryS, priceS, paramS, cS] = testCase.modelSetup(setNo);
      tau = 0;
   ...
   %% Schooling
   function schoolTest(testCase, setNo)
      [hhS, countryS, priceS, paramS, cS] = testCase.modelSetup(setNo);
      tau = 0;
